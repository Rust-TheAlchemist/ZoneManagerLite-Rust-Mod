...
{
    [Info("Zone Manager Lite", "TheAlchemist based on k1lly0u v3.1.4", "0.0.1")]
    [Description("An advanced management system for creating in-game zones")]
    public class ZoneManagerLite : RustPlugin
    {
        #region Fields
        ...
        private static ZoneManagerLite Instance { get; set; }
        private const int PLAYER_MASK = 131072;
        private const int TARGET_LAYERS = ~(1 << 10 | 1 << 18 | 1 << 28 | 1 << 29);
        private const string PERMISSION_ZONE = "zonemanagerlite.zone";
        private const string PERMISSION_IGNORE_FLAG = "zonemanagerlite.ignoreflag.";
...
//            adminBypass.SetFlags(ZoneFlags.NoBuild, ZoneFlags.NoDeploy, ZoneFlags.NoCup, ZoneFlags.NoUpgrade, ZoneFlags.NoChat, ZoneFlags.NoVoice, ZoneFlags.KillSleepers, ZoneFlags.EjectSleepers, ZoneFlags.NoSignUpdates);
            adminBypass.SetFlags(ZoneFlags.KillSleepers, ZoneFlags.EjectSleepers);
...
*/
        ...
        #region UpdateQueue // All commented out by TheAlchemist
/*	
        ...
        private UpdateBehaviour updateBehaviour
        {
            get
            {
                ...
                {
                    m_UpdateBehaviour = new GameObject("ZoneManagerLite.UpdateBehaviour").AddComponent<UpdateBehaviour>();
...
//        private void OnPlayerSleepEnd(BasePlayer player) => updateBehaviour.QueueUpdate(player);
        ...
        #region Component // class Zone : MonoBehaviour
        ...
        public class Zone : MonoBehaviour
        {
            ...
            private void Awake()
            {
                ...
                gameObject.name = "ZoneManagerLite";
                enabled = false;
...
//                        ServerMgr.Instance.StartCoroutine(ToggleLights(false));
            ...
            public void FindZoneParent()
            {
                ...
                if (Instance == null)
                {
                    Debug.LogError($"[ZoneManagerLite] Zone attempted to find parent zone, but plugin instance is null...");
                    return;
                }
...
*/
            ...
            private void RegisterPermission()
            {
                if (Instance == null)
                {
                    Debug.LogError($"[ZoneManagerLite] Zone attempted to register permission, but plugin instance is null...");
                    return;
                }
...
*/
            ...
            #region Autolights // entire region commented out by TheAlchemist
/*
...
*/
            ...
            #region IO Power  // entire region commented out by TheAlchemist
/*
            private void IOTick()
            {
...
*/			
            ...
            public bool HasPermission(BasePlayer player)
            {
                if (Instance == null)
                {
                    Debug.LogError($"[ZoneManagerLite] Zone attempted to check player permission, but plugin instance is null...");
                    return false;
                }
...
//            zone.OnEntityExitZone(baseEntity, !entityZones.HasFlag(ZoneFlags.NoDecay));
            zone.OnEntityExitZone(baseEntity, true);
        ...
        private bool CanBypass(string playerId, int flag)
        {
            ...
            Debug.LogError($"[ZoneManagerLite] CanBypass called with invalid flag : {flag}");
            return false;
        }
        ...
        #endregion Helpers for "Is" and "Can"
        ...
        private bool HasFlag(string zoneId, string flagName)
        {
            ...
            {
                Debug.Log($"[ZoneManagerLite] A plugin has call HasFlag with a invalid flag : {flagName}");
                return false;
            }
        ...
        private void AddFlag(string zoneId, string flagName)
        {
            ...
            if (ZoneFlags.NameToIndex.TryGetValue(flagName, out v))
                zone.AddFlag(v);
            else Debug.Log($"[ZoneManagerLite] A plugin has call AddFlag with a invalid flag : {flagName}");           
        }
        ...
        private void RemoveFlag(string zoneId, string flagName)
        {
            ...
            if (ZoneFlags.NameToIndex.TryGetValue(flagName, out v))
                zone.RemoveFlag(v);
            else Debug.Log($"[ZoneManagerLite] A plugin has call RemoveFlag with a invalid flag : {flagName}");
        }
        ...
        private bool HasDisabledFlag(string zoneId, string flagName)
        {
            ...
            if (!ZoneFlags.NameToIndex.TryGetValue(flagName, out v))                
            {
                Debug.Log($"[ZoneManagerLite] A plugin has call HasDisabledFlag with a invalid flag : {flagName}");
                return false;
            }
        ...
        private void AddDisabledFlag(string zoneId, string flagName)
        {
            ...
            else Debug.Log($"[ZoneManagerLite] A plugin has call AddDisabledFlag with a invalid flag : {flagName}");            
        }
        ...
        private void RemoveDisabledFlag(string zoneId, string flagName)
        {
            ...
            else Debug.Log($"[ZoneManagerLite] A plugin has call RemoveDisabledFlag with a invalid flag : {flagName}");            
        }
...
*/
        ...
        private bool EntityHasFlag(BaseEntity baseEntity, string flagName)
        {
            ...
            if (!ZoneFlags.NameToIndex.TryGetValue(flagName, out v))
            {
                Debug.LogError($"[ZoneManagerLite] A plugin has called EntityHasFlag with a invalid flag : {flagName}");
                return false;
            }
...
*/
            ...
            public bool HasFlag(string flagName)
            {
                ...
                {
                    Debug.LogError($"[ZoneManagerLite] ZoneFlags.HasFlag used with invalid flag string : {flagName}");
                    return false;
                }
                return this[v];
            }
            ...
            public void AddFlag(string flagName)
            {
                int v;
                if (!NameToIndex.TryGetValue(flagName, out v))                    
                    Debug.LogError($"[ZoneManagerLite] ZoneFlags.AddFlag used with invalid flag string : {flagName}");
                else this[v] = true;
            }
            ...
            public void RemoveFlag(string flagName)
            {
                int v;
                if (!NameToIndex.TryGetValue(flagName, out v))                   
                    Debug.LogError($"[ZoneManagerLite] ZoneFlags.RemoveFlag used with invalid flag string : {flagName}");
                else this[v] = false;
            }
            ...
            public void SetFlags(params string[] array)
            {
                ...
                for (int i = 0; i < array.Length; i++)
                {
                    ...
                    if (NameToIndex.TryGetValue(array[i], out v))                        
                        Debug.Log($"[ZoneManagerLite] ZoneFlags.SetFlags used with invalid flag string : {array[i]}");
                    else list.Add(v);
                }
...
*/
            if (HasGlobalFlag(ZoneFlags.SleepGod) || HasGlobalFlag(ZoneFlags.PvpGod) || HasGlobalFlag(ZoneFlags.PveGod) || HasGlobalFlag(ZoneFlags.NoPve) || HasGlobalFlag(ZoneFlags.UnDestr) || HasGlobalFlag(ZoneFlags.NoBuildingDamage))
                Subscribe(nameof(OnEntityTakeDamage));
...
*/
            ...
            if (HasGlobalFlag(ZoneFlags.LootSelf) || HasGlobalFlag(ZoneFlags.NoPlayerLoot) || HasGlobalFlag(ZoneFlags.NoBoxLoot))
                Subscribe(nameof(CanLootEntity));
            ...
            for (int i = 0; i < args.Length; i = i + 2)
            {
                ...
                switch (args[i].ToLower())
                {
                    ...
                    case "permission":
                        ...
                        if (!permission.StartsWith("zonemanagerlite."))
                            permission = $"zonemanagerlite.{permission}";
...
//                OpenFlagEditor(player, zoneId);
        ...
        #region UI  // commenting entire region out for now by TheAlchemist
/*		
...
*/		
        ...
        {
            data = Interface.Oxide.DataFileSystem.GetFile("ZoneManagerLite/zone_data");
